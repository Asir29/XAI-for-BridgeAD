...
class Sparse4DHead(BaseModule):
    ...
    def forward(
        ...
        batch_size = feature_maps[0].shape[0]  # 8  # 6
        ...
        (
            instance_feature,  # torch.Size([8, 900, 256]) torch.Size([8, 100, 256])
            anchor,  # torch.Size([8, 900, 11]) torch.Size([8, 100, 40])
            temp_instance_feature,  # torch.Size([8, 600, 256])  # torch.Size([6, 33, 256])
            temp_anchor,  # torch.Size([8, 600, 11])  # torch.Size([6, 33, 40])
            time_interval,  # torch.Size([8])  # torch.Size([6])
        ) = self.instance_bank.get(
            batch_size, metas, dn_metas=self.sampler.dn_metas
        )

        # map module did not use temporal information in stage1?
        # As you can see in StreamMapNet, 
        # starting with several epochs of single frame training 
        # will provide a good initialization for temporal propagation.
        ...
        anchor_embed = self.anchor_encoder(anchor)  # torch.Size([8, 900, 256])
        if temp_anchor is not None:
            temp_anchor_embed = self.anchor_encoder(temp_anchor)  # torch.Size([8, 600, 256])
        else:
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                quality.append(qt)
                if len(prediction) == self.num_single_frame_decoder:  # when 1, update
